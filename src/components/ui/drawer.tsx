	...
	(
		...
		return (
			<>
				<HDrawer
					...
					classNames={{
						...
						base: cn('w-[var(--sidebar-width)] !m-0 p-0 h-full max-h-full', classNames?.base, className, {
							'inset-y-0 left-5 max-h-[none] rounded-l-none !justify-start': sidebarPlacement === 'left',
							'inset-y-0 right-5 max-h-[none] rounded-r-none !justify-end': sidebarPlacement === 'right',
						}),
				...
				<div className={cn('h-full max-w-[var(--sidebar-width)] overflow-x-hidden overflow-y-scroll flex', className)}>
					{children}
				</div>
			</>
		);
